...
def realizzazione(W):
	...
	n = degree(Dw)
	out = "\[ W(s) = \\frac{%s}{%s} \]\n"%(l(Nw),l(Dw))
	out+= f"Il sistema ha dimensione $q: {q},p: {p}$, inoltre il grado del denominatore è ${n}$.\n"



	matrici = {d : zeros(q,p) for d in range(n)}
	for i in range(q*p):
		c = coeffs(Nw[i])[::-1]
		for deg in range(len(c)):
			matrici[deg][i] = c[deg]
	
	str_can_ragg,Ar,Br,Cr = better_can_ragg(q,p,matrici,Dw)
